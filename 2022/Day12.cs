...
public class Day12 : Day<Day12.Map>
{
    ...
    private List<(int, int)> Search(char endLevel)
    {
        var startNode = new Node() { Position = Input.Start() };
        var endNodes = Input.End(endLevel);
        ...
        while (open.Any())
        {
            ...
            foreach (var dir in dirs)
            {
                ...
                var neighborLevel = Input.Grid[newY][newX] == endLevel ? endLevelInt : Input.Grid[newY][newX];
    ...
    {
        return Search('S').Count-1;
    }

    protected override long Part2()
    {
        return Search('a').Count-1;
    }
